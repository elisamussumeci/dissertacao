...
$X_{D-d}$ is the vector with the values of all predictor series at time $D-d$, including $y$. The model predicts the incidence at a particular week in the future $y_{D+\tau}$, thus since we wanted to predict 4 weeks into the future, 4 separate models were fitted to data with $\tau$ varying from 1 to 4.
...
\end{figure}
 ...
 \caption{Features matrix example from Random Forest}
 \label{fig:rf_matrix}
\end{figure}
